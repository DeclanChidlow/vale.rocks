...
While it is true that people will mainly use the popular offerings, there are so many models out there -- each with their own distinctions -- such that it is impossible to identify patterns for each of them. Even if you could, it would be an endless task, as new models are released continuously, and one could feasibly train their own or gaolbreak or fine-tune an existing model to evade detection.

People often point to writing features such as the em dash (—), tricolons/triads, or emoji list as proof of text's AI origin, forgetting that AI models are trained on human output and that all of the aforementioned have long been features of content in AI's training corpora. Some people take the presence of these writing features to such an extreme that they vocally and entirely unnecessarily [attack writers despite them not using AI](https://wil.to/posts/llemdashes/).
...
Good AI-generated speech can be a little uncanny if you actively listen for it but are largely beyond the hump now and indistinguishable at the cutting edge.
...
Still, certain models have specific tells. For example, [OpenAI's 4o Image Generation](https://openai.com/index/introducing-4o-image-generation/) has a tendency to apply a warm, sepia-tone filter of sorts over everything it generates, and this becomes more exaggerated as you request further refinements and it further yellows images. The following release, [ChatGPT Images 2.0](https://openai.com/index/introducing-chatgpt-images-2-0/), retained the yellowing problem, though to a much lesser extent.

Where the tells aren't just being aggressively mitigated, some models don't have any specific tells at all. There is no distinct 'look' attributable to imagery generated by them, and some advanced models such as Google's Nano Banana maintain consistency across revisions in such a way that they can output AI-altered versions of genuine photography (though with some very minor changes), further muddying the waters.
...
Music is already less 'obvious' than images. It is more abstract, and we're used to the autotune, artificial sounds, and high processing that would be equivalent to tells in other mediums. It is also not able to be evaluated all at once due to being a moving presentation. One cannot pause a song or 'zoom in' to analyse a specific part as one can a video or image.

As of mid-2026, high-quality AI-generated music is difficult to detect by ear but can be reliably identified via digital analysis. The remaining tells that can sometimes be identified by humans are usually related to the voice, such as the singer never taking breath.
...
The [Coalition for Content Provenance and Authenticity (C2PA)](https://c2pa.org) does exist and has members including OpenAI, Meta, Google, Microsoft, and plenty of other AI companies but is more or less useless for acting as a definitive source for whether something is AI-generated.

OpenAI [notes in their policy](https://help.openai.com/en/articles/8912793-c2pa-in-chatgpt-images) that:
...
It is worth noting that most existing image formats lack any secure way to store metadata without the ability to tamper with it. Even if you can make the metadata itself untamperable, one could always just screenshot or take a picture of it, creating a new image devoid of that metadata. Further, if at any point that format of storing metadata was broken or cracked, the validity of all previous images would be immediately thrown into question.
...
OpenAI has also dabbled with visual watermarks, with small coloured blocks in the bottom corner of their DALL-E output, but they were easily removed or avoided. OpenAI didn't bring this visual watermark feature back for future image models either. Images generated with Gemini have a small watermark in the bottom left, but that is also easily removed.

A better approach is [Google's SynthID](https://deepmind.google/science/synthid/), which uses stylometry to weave watermarks into the output itself in such a way that humans don't notice it, but even that is not a perfect solution. Methods have been devised to bypass SynthID, though that hasn't stopped adoption of the technology by Nvidia, Kakao, OpenAI, and ElevenLabs.

Text can be 'watermarked' by altering the probabilities of certain words, but this isn't sure-fire and can be circumvented by back-translating the text, paraphrasing it, or employing one of a number of other transformations. It also won't work forever, given language's fluidity in adopting AI styling. It is possible for the file containing the text to have its own metadata, but it is all too easy to copy or transcribe text without carrying over metadata.[^2]
...
People using AI output in many cases do not wish to have it identifiable as AI output -- especially given the negative reputation AI output has for being slop and the potential backlash that brings with it. There are many con artists making substantial financial gains peddling AI output who I'd suggest would be willing to go to lengths to avoid people noticing.[^3]

Companies building generative AI offerings may not wish to have their output identifiable as AI, especially if their competitors don't identify their output, which could prove a selling point in some cases. I have thoughts that a lot of watermarking measures are intentionally flawed, due to them being much more an effort to appease regulators and bolster public relations than to curb misinformation. After all, one can also just use any of the many capable image generators without such watermarking built in.

I've also seen the argument made that content that is modified by image editing software like Adobe Photoshop is not marked in any meaningful way, so AI output shouldn't need to be either.
